...
$(function () {
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      var story_prototype = T.User.object_prototypes
        .filterTypes("story")
        .getAll()[0];
      ...
      if (story_prototype) {
        ...
        for (var i = 0; i < files.length; i++) {
          ...
          if (files[i].size > TOWNS_CDN_FILE_MAX_SIZE) {
            T.UI.Message.error(
              T.Locale.get("upload error max filesize") +
                " " +
        ...
        if (request_size > TOWNS_CDN_REQUEST_MAX_SIZE) {
          T.UI.Message.error(
            T.Locale.get("upload error max requestsize") +
              " " +
        ...
        xhr.upload.onprogress = function (event) {
          if (event.lengthComputable) {
            ...
            message.text(
              T.Locale.get("upload progress") + " " + complete + "%"
            );
          }
        };
        ...
        xhr.onload = function () {
          if (xhr.status === 200) {
            try {
              ...
              r(message);
              message
                .text(T.Locale.get("upload success story"), "success")
                .close();
            } catch (e) {
