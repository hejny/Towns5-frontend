  ...
  static menu() {
    ...
    if (map_selected_ids.length > 0) {
      ...
      ['view', 'edit'].forEach(function(action) {
        var possible = T.Plugins.search(
            action, T.ArrayFunctions.id2item(objects_external, id));
        possible = possible.map(function(item) {
          return (`<button onclick="T.Plugins.open('` + item + `',1,'` + id +
                  `')">` +
        ...
        objectmenu += T.Templates.menu({
          ...
          content :
              T.Locale.get(object.type, object.subtype, action, 'description') +
                  '<br>' + possible
        });
      });
      /**/
      ...
      objectmenu += T.Templates.menu({
        ...
        content :
            T.Locale.get(object.type, object.subtype, 'clone', 'description'),
      ...
      objectmenu += T.Templates.menu({
        ...
        content : T.Locale.get(object.type, object.subtype, 'dismantle',
                               'description'),
