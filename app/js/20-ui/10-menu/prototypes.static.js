  ...
  static menu(type, subtype = false) {
    ...
    if (type == 'building' && subtype == 'block') {
      object_menu_html += T.Templates.menu({
        ...
        content : content,
        action :
            `mapSpecialCursorStop();T.Plugins.open('building-block-editor',0,-1);`
      });
    }
    ...
    T.User.object_prototypes.forEach(function(object) {
      if (object.type == type && (!is(subtype) || object.subtype == subtype)) {
        ...
        var icon, icon_size = 1, title = object.name, content = '', action = '';
        ...
        if (object.type == 'building') {
          ...
          action = 'T.UI.Menu.Building.start(\'' + object.id +
                   '\');'; // todo refactor all object.id to object._id
        ...
        if (object.type == 'terrain') {
          ...
          if (object.getCode() == 14)
            return; // todo better

          icon = appDir + '/php/terrain.php?raw&size=60&terrain=t' +
                 (object.getCode());
          action = 'T.UI.Menu.Terrains.start(\'' + object.id + '\');';
          icon_size = 1.2;

          title = T.Locale.get('object terrain', object.getCode());
          content += '<img width="280" height="140" src="' +
                     (appDir + '/php/terrain.php?raw&size=280&terrain=t' +
                      (object.getCode())) +
                     '">';
        ...
        ['view', 'edit'].forEach(function(action) {
          ...
          possible = possible.map(function(item) {
            return (`<button onclick="T.Plugins.open('` + item + `',0,'` +
                    object.id + `')">` +
                    T.Locale.get('plugin', item, 'open', object.type,
                                 object.subtype, action) +
                    `</button>`);
          });
