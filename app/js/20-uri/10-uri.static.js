...
T.URI = class {
  ...
  static read() {
    ...
    if (hash.length == 2) {
      ...
      position =
          new T.Position(T.Math.toFloat(hash[0]), T.Math.toFloat(hash[1]));
    }
      ...
      if (this.plugin) {
        ...
        if (!T.Plugins.is(this.plugin)) {
          ...
          T.UI.popupWindow.open(T.Locale.get('page', '404', 'title'),
                                T.Locale.get('page', '404', 'content'), false,
                                'SMALL');
          return;
        }
  ...
  static write() {
    ...
    var pathname = '';
    if (this.plugin)
      pathname += '/' + this.plugin;
    if (this.object && this.plugin)
      pathname += '/' + this.object;
    ...
    if (isDefined(T.UI.Map.map_center.x)) {
      // var hash =
      // '#'+Math.round(T.URI.position.x)+','+Math.round(T.URI.position.y);
      hash = '#' + Math.round(T.UI.Map.map_center.x) + ',' +
             Math.round(T.UI.Map.map_center.y);
    } else {
      hash = '';
    }

    window.history.pushState('', "Towns",
