 ...
 * todo write
 */
T.Model.prototype.drawCashedAsync = function(
    ctx, s, x_begin, y_begin, rotation, slope, selected = false, shadow = false,
    clearBefore = false) {
  ...
  var draw = function() {
    /*CanvasRenderingContext2D.prototype.drawImageAntialias =
    function(image,x,y,width,height){
        ...
        //Step 2 reuses the off-screen canvas and draws the image reduced to
    half again: octx.drawImage(oc, 0, 0, oc.width * 0.5, oc.height * 0.5);


        // step 3 - And we draw once more to main canvas, again reduced to half
    but to the final size: ctx.drawImage(oc, 0, 0, oc.width * 0.5, oc.height *
    0.5, 0, 0, canvas.width,   canvas.height);
    ...
    if (selected) {
      ...
      ctx.drawImage(canvas_, x_begin - (width / 2), y_begin - (height / 2),
                    width, height);
    }

    ctx.drawImage(image, x_begin - (width / 2), y_begin - (height / 2), width,
                  height);
...
T.Model.prototype.createCacheLocalImage = function(size, rotation, slope) {
  ...
  if (!isDefined(BuildingImages[hash])) {
    ...
    var src = createCanvasViaFunctionAndConvertToSrc(500, 500, function(gl) {
      // r(self);
      var webGL =
          self.create3D(gl, size, 250, 250, rotation, slope, false, true);
...
/*todo create url
Model.prototype.createSrc = function( s, x_begin, y_begin, x_size, y_size, rot,
