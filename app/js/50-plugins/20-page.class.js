...
T.Plugins.Page = class {
   ...
   * @constructor
   */
  constructor(uri, title, content, open_callback = false,
              close_callback = false, format = 'NORMAL') {
  ...
  open(additional_callback = false, additional_parameters = false) {
    ...
    if (this.titles.length == 1) {
      ...
      title = this.titles
                  .map(function(title, i) {
                    ...
                    title = '<li child="' + i + '">' + title +
                            '</li>'; // todo use XML namespaces
    ...
    if (this.contents.length == 1) {
      ...
      content = this.contents
                    .map(function(content, i) {
                      content = '<article class="page-child" id="page-child-' +
                                i + '" style="display: none;">' + content +
                                '</article>'; // todo use XML namespaces
    ...
    if (this.titles.length != 1) {
      ...
      $('#page-choose li').click(function() {
        ...
        // todo effects
        $('.page-child')
            .not('#page-child-' + page_child)
            .hide(/*"slide", { direction: "left" }, 1000*/); //.hide();
        $('#page-child-' + page_child)
    ...
    if (this.open_callback) {
      setTimeout(function() {
        ...
        // todo refactor not DI popup window content but use static container
        // with function T.ui.get();
      }, IMMEDIATELY_MS);
    }
