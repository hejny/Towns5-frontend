...
T.Plugins.Viewer = class {
  ...
  constructor(uri, conditions, title, content, open_callback) {
    ...
    var self = this;
    this.page = new T.Plugins.Page(uri, title, `
  ...
  open(collection, id) {
      ...
      viewer.page.open(function(open_callback, object) {
        ...
        open_callback(object,
                      $('.popup-window .content')
                          [0]); // todo refactor not DI popup window content but
        ...
        $('#viewer-object-edit').click(function(e) {
          if (object.type == 'story') {
            ...
            T.Plugins.open(
                'story-editor', 1,
                object.id); // todo better switching between viewers and editors
    ...
    if (collection === 0) {
      ...
      viewer.opened.object = T.User.object_prototypes.getById(
          id); // T.ArrayFunctions.id2item(T.User.object_prototypes,id);
      ...
      viewer.opened.object = objects_external.getById(
          id); // T.ArrayFunctions.id2item(objects_external,id);

      if (viewer.opened.object) {

        r('Opening object ' + viewer.opened.object.name +
