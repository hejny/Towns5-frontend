...
T.Plugins.Editor = class {
  ...
  constructor(uri, conditions, title, content, open_callback, default_object) {
    ...
    this.page = new T.Plugins.Page(
        ...
        <form id="editor-object-header">
            <input type="text" id="editor-object-name" value="" placeholder="{{` +
            default_object.type + ` ` + default_object.subtype +
            ` name placeholder}}">


            <div class="button-icon"  id="editor-object-duplicate" title="{{` +
            default_object.type + ` ` + default_object.subtype +
            ` duplicate}}"><i class="fa fa-clone"></i></div>


            <div class="button-icon"  id="editor-object-delete" title="{{` +
            default_object.type + ` ` + default_object.subtype +
            ` delete}}"><i class="fa fa-trash-o"></i></div>


        </form>
        <div id="editor-object-errors"></div>` +
            content,
        false, function() {
          ...
          if (self.opened.collection === 0) {
            ...
            T.User.object_prototypes.setById(self.opened.object.id,
                                             self.opened.object);

            T.UI.Menu.Prototypes.menu(self.opened.object,
                                      self.opened.object.subtype);
            T.UI.Menu.Building.start(self.opened.object.id);

            T.TownsAPI.townsAPI.post(
                'objects/prototypes', self.opened.object,
                function(response) {
                  T.UI.Menu.Prototypes.menu(self.opened.object.type,
                                            self.opened.object.subtype);
                  T.UI.Menu.Building.start(self.opened.object.id);
                },
                function(
                    errors) { self.open(0, self.opened.object.id, errors); });
            ...
            T.TownsAPI.townsAPI.post(
                'objects/' + self.opened.object.id, self.opened.object,
                function(response) {
                  T.UI.Message.success(
                      T.Locale.get('object', self.opened.object.type, 'saved'));
                },
                function(errors) {
                  r(self.opened);
  ...
  open(collection, id, errors = false) {
    ...
    this.opened = {collection : collection};
      ...
      editor.page.open(function(open_callback, object) {
        ...
        if (errors) {
          ...
          for (var key in errors.message) {
            ...
            $('#editor-object-errors')
                .append('<div class="error">' +
                        (errors.message[key].message.text2html()) + '</div>');
          }
        }
        ...
        $('#editor-object-delete').click(function() {
          if (editor.opened.collection === 0) {
            ...
            if (confirm(T.Locale.get(
                    'delete prototype ' + object.type + ' ' + object.subtype +
                    ' confirm'))) { // todo create better confirm
          ...
          } else if (editor.opened.collection === 1) {

            r('Deleting object ' + object.name + '.');

            // todo maybe create action DELETE prototype?
            if (confirm(
                    T.Locale.get(object.type, 'delete',
                                 'confirm'))) { // todo create better confirm
            ...
            throw new Error('' + collection +
                            ' is invalid identificator of collection!');
          }
        });
        //-----------------
        ...
        if (collection === 0) {
          ...
          $('#editor-object-duplicate').show().click(function() {
            if (editor.opened.collection === 0) {
              ...
              if (confirm(
                      T.Locale.get('duplicate prototype ' + object.type + ' ' +
                                   object.subtype +
                                   ' confirm'))) { // todo create better confirm
    ...
    if (id == -1) {
      ...
      if (collection === 0) {
        ...
        T.UI.Menu.Prototypes.menu(this.opened.object.type,
                                  this.opened.object.subtype);
        ...
        throw new Error(
            'In objects can not be created new object without prototype.');
        ...
        if (editor.opened.object) {
          ...
          T.TownsAPI.townsAPI.get(
              ...
              function(response) {
                ...
                r('Opening object ' + editor.opened.object.name +
                  ' loaded from API.');
                object_ready();
              },
              function() {
                T.UI.popupWindow.open(T.Locale.get('page', '404', 'title'),
