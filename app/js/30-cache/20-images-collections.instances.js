...
T.Cache.backgrounds = new T.Cache.ImagesCollection(
  files,
  appDir + "/php/terrain.php?terrain="
);
...
for (var seed = 0; seed < rockCount; seed++) {
  for (var dark = 0; dark < rockCountDark; dark++) {
    files["rock" + seed + "dark" + dark] =
      ...
      seed +
      "&width=133&dark=" +
      Math.round((dark / rockCountDark) * rockMaxDark);
  }
}

//----------------

T.Cache.objectsNatural = new T.Cache.ImagesCollection(
  files,
  appDir + "/php/treerock.php"
);
...
T.Cache.imageLoad = function () {
  ...
  T.Cache.message.text(
    T.Locale.get("loading of graphic") + " " + Math.floor(percent * 100) + "%"
  );
