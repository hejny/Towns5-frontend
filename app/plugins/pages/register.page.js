...
T.Plugins.install(new T.Plugins.Page(
    'register', T.Locale.get('page', 'register'),
    `
...
<div id="login-error" class="hidden"></div>
 ...
 <div id="register-form" class="form-big">
    ...
    <table>
        ...
        `" value="" autofocus autocomplete="off"></td>
        </tr>


        <tr>
            <td>*` +
        T.Locale.get('user', 'password') + `:</td>
            <td><input class="js-townsapi-online" type="password" name="password" autocomplete="off"></td>
        </tr>
        <tr>
            <td>*` +
        T.Locale.get('user', 'password', 'again') + `:</td>
...
`*/ `
        ...
        T.Locale.get('user', 'register') + `</button>
            </td>
        </tr>
    ...
    function(page) {
      ...
      $(page).find('button').click(function(e) {
        ...
        if (data.username.trim().length === 0) {
          ...
          setInputError($("input[name='password']")[0],
                        T.Locale.get('user', 'password', 'missing'));
          $(page)
              .find('.messages')
              .html('<div class="error messagebox">' +
                    T.Locale.get('user', 'password', 'missing') + '</div>');

        } else
            //---------------------------------------------
            if (data.password !== data.password_again) {

          setInputError($("input[name='password-again']")[0],
                        T.Locale.get('user', 'password', 'again', 'notsame'));
          $(page)
              .find('.messages')
              .html('<div class="error messagebox">' +
                    T.Locale.get('user', 'password', 'again', 'notsame') +
          ...
          T.TownsAPI.townsAPI.post(
              ...
              function(response) {
                T.TownsAPI.townsAPI.post(
                    'auth', {"username" : data.username, "password" : password},
                    function(response) {
                      $(page).find('button').html(
              ...
              function(response) {
                ...
                $(page)
                    ...
                    .html('<div class="info messagebox">' +
                          T.Locale.get('user', 'username', 'taken') + '</div>');
              });
